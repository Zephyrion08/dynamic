...
function template($filename, $vars, $current="orion")
{
    if (file_exists($filename)) {
        ...
        // Add base paths for template assets
        ...
        // 🔹 Global underscore fix for <a> tags only
        $output = preg_replace_callback(
            ...
            function ($matches) {
                ...
                // Replace underscore with hyphen for SEO version
                $clean = str_replace('_', '-', $url);

                // Rebuild <a> tag with both attributes
                return "<a {$before}href=\"{$clean}\" data-original-href=\"{$url}\"{$after}>";
            },
            $output
        );

        // 🔹 Auto-inject maincustom.js before </body>
        $jsPath = BASE_URL . 'js/maincustom.js';
        $scriptTag = "\n<script src=\"{$jsPath}\"></script>\n";

        if (stripos($output, '</body>') !== false) {
            // Insert before </body>
            $output = preg_replace('/<\/body>/i', $scriptTag . '</body>', $output, 1);
        } else {
...
function getMenuList($caption="", $link="", $type=0, $class="",$nicon="",$attribute=""){
	...
	return "<a href=\"".$linkhref."\" ".$classLink.$idLink.$linkType." ".$attribute.">".$caption."".$nicon."</a> ";
}
function getMenuListchild($caption="", $link="", $type=0, $class="",$nicon="",$attribute=""){
	/*$classLink 	= ($class == '') ? '' : 'class="show-submenu';*/
	$classLink 	= ($class == '') ? '' : 'class="'.$class.' "';
	$linkhref	= ($link == '#') ? 'javascript:void(0);' : $link;
	$base 		= ($linkhref=='javascript:void(0);')?'':BASE_URL;
	...
	$linkType	= ($type == 0) ? '' : ' target="_blank"';
	$linkhref	= ($type == 0) ? BASE_URL.$link : $link;
	return "<a href=\"".$linkhref."\" ".$classLink.$idLink.$linkType." ".$attribute."><span>".$caption."</span></a>".$nicon." ";
}
...
function get_front_pagination($total = '', $per_page = '2', $page = '1', $url = '')
{
    ...
    $pagination = "";
    if ($lastpage > 1) {
        // wrapper as in your static code
        $pagination .= "<div class='col-md-12'>";
        $pagination .= "<ul class='news-pagination-wrap align-center mb-30 mt-30'>";

        // Prev button
        if ($page > 1)
            $pagination .= "<li><a href='" . $url . "/page/" . $prev . "'><i class='ti-angle-left'></i></a></li>";
        else
            $pagination .= "<li><a href='javascript:void(0);'><i class='ti-angle-left'></i></a></li>";
        ...
        // Next button
        if ($page < $counter - 1)
            $pagination .= "<li><a href='" . $url . "/page/" . $next . "'><i class='ti-angle-right'></i></a></li>";
        else
            $pagination .= "<li><a href='javascript:void(0);'><i class='ti-angle-right'></i></a></li>";
