...
class Services extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug','title', 'sub_title', 'image', 'icon', 'linksrc','brief', 'linktype', 'content', 'status', 'added_date', 'sortorder', 'type', 'meta_title', 'service_type','meta_keywords', 'meta_description');
	...
	public static function getservice_list($limit='',$type=1) {
		...
		$sql="SELECT * FROM ".self::$table_name." WHERE status='1' AND service_type=$type ORDER BY sortorder ASC $lmt";
	...
	public static function checkDupliName($title='')
	{
		...
		global $db;		
		$sql = "SELECT slug, title FROM ".self::$table_name." WHERE status='1' AND service_type='2' ORDER BY sortorder ASC";
		...
		$result.='<optgroup label="Services">';
			foreach($pages as $pageRow):
				...
				$result.='<option value="service/'.$pageRow->slug.'" '.$sel.'>&nbsp;&nbsp;'.$pageRow->title.'</option>';
			endforeach;
		$result.='</optgroup>';	
		endif;
		return $result;
