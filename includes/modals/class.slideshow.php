...
class Slideshow extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'title','sub_title', 'image', 'linksrc','source_vid','mode', 'linktype', 'content','class','host','url_type','thumb_image','url', 'status', 'm_status', 'added_date', 'sortorder', 'type', 'source','upcoming');
    ...
    public static function getSlideshow_by($type = '',$limit='',$upcoming=0)
    {
        ...
        $cond1 = isset($upcoming) ? ' AND upcoming=' . (int)$upcoming : ''; // Fix
        $cond2 = !empty($limit) ? ' LIMIT ' . $limit : '';
        $sql="SELECT * FROM " . self::$table_name . " WHERE status=1 $cond $cond1 ORDER BY sortorder DESC $cond2";
        // pr($sql);
 ...
 public static function getSlideshow_by_mode($mode = '',$limit='',$upcoming=0)
    {
        global $db;
     ...
     public static function getslide_by_mode($mode = '')
    {
        global $db;
        $cond = !empty($mode) ? ' AND mode=' . $mode : '';
