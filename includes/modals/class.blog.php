...
class Blog extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'title', 'author','banner_image', 'brief', 'content', 'status', 'meta_keywords', 'meta_description', 'blog_date','linksrc','linktype', 'archive_date', 'sortorder', 'type', 'viewcount', 'added_date', 'image', 'source');
...
public static function get_relatedsub_by( $exclude_ids = [], $limit = '')
{
    global $db;

    $cond = '';
    if (!empty($exclude_ids)) {
        // Escape and build safe list
        $ids = array_map('intval', (array)$exclude_ids);
        $cond = ' AND id NOT IN (' . implode(',', $ids) . ')';
    }

    $cond2 = !empty($limit) ? ' LIMIT ' . intval($limit) : '';

    $sql = "SELECT * FROM " . self::$table_name . " 
            WHERE status=1 $cond 
            ORDER BY sortorder DESC $cond2";
