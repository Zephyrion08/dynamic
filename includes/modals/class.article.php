...
class Article extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'parent_id', 'slug', 'title', 'sub_title', 'linksrc', 'linktype', 'content', 'upcoming', 'status', 'sortorder', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'modified_date','homepage', 'image');
	...
	public static function checkDupliName($title='') {
		global $db;
		$query = $db->query("SELECT title FROM ".self::$table_name." WHERE title='$title' AND upcoming=0 LIMIT 1");
	...
	// Homepage Display
	public static function homepageArticle($limit='',$upcoming=0) {
		global $db;
		$lmt = !empty($limit)?' LIMIT '.$limit:'';
		$sql="SELECT * FROM ".self::$table_name." WHERE status='1' AND homepage='1' AND upcoming=$upcoming ORDER BY sortorder ASC $lmt";
		return self::find_by_sql($sql);
	}
    ...
    public static function find_all_active(){
        global $db;
        return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status=1 AND upcoming=0 ORDER BY sortorder DESC");
    }
	...
	static function find_by_slug($slug='',$upcoming=0) {
		global $db;
		$result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE slug='$slug' AND status='1' AND upcoming='$upcoming' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
	}
