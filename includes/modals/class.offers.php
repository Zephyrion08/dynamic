...
class Offers extends DatabaseObject
{
    ...
    protected static $db_fields = array(
        'id', 'slug', 'title', 'image', 'offer_date','offerpopup','start_date', 'linksrc', 'linktype', 'rate', 'discount', 'brief', 'content', 'adults', 'children',
    ...
    public static function get_internal_link($Lsel = '', $LType = 0)
    {
        ...
        if ($pages):
            ...
            foreach ($pages as $pageRow):
                $sel = ($Lsel == ("offer-list/" . $pageRow->slug)) ? 'selected' : '';
                $result .= '<option value="offer-list/' . $pageRow->slug . '" ' . $sel . '>&nbsp;&nbsp;' . $pageRow->title . '</option>';
            endforeach;
            $result .= '</optgroup>';
        endif;
        return $result;
