...
if (isset($_GET['page']) && $_GET['page'] == "blog" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    ...
    clearImages($moduleTablename, $moduleFoldername . "/thumbnails");
     clearImages($moduleTablename, "blog/banner","banner_image");
    clearImages($moduleTablename,  "blog/banner/thumbnails","banner_image");
        ...
        <div class="example-code">
            <form action="" class="col-md-12 center-margin" id="blog_frm">
                ...
                <div class="form-row">
                    ...
                    <?php if (!empty($blogInfo->banner_image)): ?>
                        <div class="col-md-3" id="removeSavedimg200">
                            <div class="infobox info-bg">
                                <div class="button-group" data-toggle="buttons">
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/blog/banner/" . $blogInfo->banner_image)):
                                    $filesize = filesize(SITE_ROOT . "images/blog/banner/" . $blogInfo->banner_image);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                    <a class="btn small float-right" href="javascript:void(0);"
                                       onclick="deleteSavedBlogimage(200);">
                                        <i class="glyph-icon icon-trash-o"></i>
                                    </a>
                                </div>
                                <img src="<?php echo IMAGE_PATH . 'blog/banner/thumbnails/' . $blogInfo->banner_image; ?>"
                                     style="width:100%"/>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div id="preview_Image2"><input type="hidden" name="imageArrayname2"
                                                   value="<?php echo !empty($blogInfo->banner_image) ? $blogInfo->banner_image : ""; ?>"
                                                   class=""/></div>
                </div>
     ...
     <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#banner_upload').uploadify({
                ...
                'formData': {
                    PROJECT: '<?php echo SITE_FOLDER;?>',
                    targetFolder: 'images/blog/banner/',
                    thumb_width: 200,
                    thumb_height: 200
                },
                'method': 'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageName').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/blog/banner_image.php', {imagefile: filename}, function (msg) {
